...
def preprocess_save_combined(alpha=0.42,
    ...
    for idx,path in enumerate(file_list):
        os.mkdir(str(idx),0o755)
        print("Category:", idx)
        files = glob.glob(path[1:-1])
        total_samples = len(files)
        ...
        for k,fname in tqdm.tqdm(enumerate((files)), total=len(files)):
            if process_type[idx] == "mngu0":
                data = ema_read(fname)
                data = resample(data,int(np.ceil(data.shape[0]*2/5)))
            if process_type[idx] == "timit":
                data = ema_read_mocha(fname)
            if process_type[idx] == "torgo":
                data, start_calib = ema_read_tor(fname,start_calib)

            # We don't need the time and present rows
            tmax = np.minimum(data.shape[0],max_length)
            dataset[k,:tmax,:-1] = data[:tmax,:]

            fname_wo_extension = fname[:-3 or None] + "wav"
            sound_data, fs = sf.read(wav_path)

            # Stereo exception handling
...
if __name__ == "__main__":
    ...
    p.add('--file_list', action="append", required=False, help='Files to process')
    p.add('--process_type', action="append", required=False, help='mngu0/torgo/timit')
    p.add('--proc', action="store_true", required=False, help='Start processing')
    p.add('--save_dir', required=False, help='Location to save preprocessed dataset')
    args = p.parse_args()
    if (args.proc):
        preprocess_save_combined(alpha=0.42,
                                 max_length=1000, fs=16000, val_split=0.2,
                                 noise=False,combined=False, bins_1 = 41,
                                 bins_2 = 1, normalisation_input = True,
                                 normalisation_output = True,
